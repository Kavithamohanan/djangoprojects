...
def add_to_cart(request,p):
    product=Product.objects.get(name=p)#retrieved clicked product details from database
    u=request.user   #retrived the name of current login user
    try:
        cart=Cart.objects.get(user=u,product=product)#Checking the product is already in the cart
        if(cart.quantity<cart.product.stock): # condition:if quantity is less than stock give+1
             cart.quantity+=1 #if present incriment the quantity
        cart.save()
